...
BRANCH_NAME = os.getenv("GITHUB_BRANCH", "results")
...
def push_project(commit_message: str = "Mise à jour complète du projet multi-agents",
    ...
    original_gitignore = GIT_CWD / ".gitignore"
    backup_gitignore = GIT_CWD / ".gitignore.bak"
    strict_gitignore_content = """*
!.gitignore
!output/
!output/**
"""
    # Save current branch
    current_branch = get_current_branch(GIT_CWD)
    try:
        # Backup original .gitignore if exists
        if original_gitignore.exists():
            original_gitignore.replace(backup_gitignore)
        ...
        if backup_gitignore.exists():
            backup_gitignore.replace(original_gitignore)
        # Restore original branch
        if current_branch and current_branch != branch:
            try:
                switch_or_create_branch(current_branch, GIT_CWD)
            except Exception as e:
                print(f"Erreur lors du retour à la branche d'origine: {e}")
    return result
